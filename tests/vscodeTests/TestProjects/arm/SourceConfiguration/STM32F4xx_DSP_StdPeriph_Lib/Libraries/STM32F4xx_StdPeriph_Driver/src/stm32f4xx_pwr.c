...
@verbatim   
 ...
 ===============================================================================  
    [..]
      ...
      (+) For STM32F40xx/STM32F41xx Devices, the regulator can be configured on the    
          fly through PWR_MainRegulatorModeConfig() function  which configure VOS bit 
          in PWR_CR register:
       ...
       (+) For STM32F427x/STM32F437x/STM32F429x/STM32F439x Devices, the regulator can     
           be configured through PWR_MainRegulatorModeConfig() function which configure 
           VOS[1:0] bits in PWR_CR register:  
       ...
       (+) For STM32F427x/STM32F437x/STM32F429x/STM32F439x Devices, the scale can be 
           modified only when the PLL is OFF and the HSI or HSE clock source is selected 
           as system clock. The new value programmed is active only when the PLL is ON.
           When the PLL is OFF, the voltage scale 3 is automatically selected. 
        Refer to the datasheets for more details.
        
       (+) For STM32F429x/STM32F439x Devices, in Run mode: the main regulator has
       ...
       (+) For STM32F429x/STM32F439x Devices, in Stop mode: the main regulator or low power regulator 
...
/**
  ...
  *                                                System frequency up to 120 MHz (only for STM32F427x/437x/429x/439x devices)
  * @retval None
  */
...
/**
  ...
  * 
  * @note   This function can be used only for STM32F429x/STM32439x devices.
    ...
    [..]
      ...
      (+) Entry:
        (++) The Stop mode is entered using the PWR_EnterSTOPMode(PWR_MainRegulator_ON) 
             function with:
          (+++) Main regulator ON.
          (+++) Main regulator in low voltage (available only for STM32F429x/439x devices). 
          (+++) Low Power regulator ON.
          (+++) Low Power regulator in low voltage (available only for STM32F429x/439x devices).
      (+) Exit:
...
/**
  ...
  *            @arg PWR_MainRegulator_LowVoltage: STOP mode with main regulator in low voltage mode (Only STM32F429x/439x devices)
  *            @arg PWR_LowPowerRegulator_LowVoltage: STOP mode with low power regulator in low voltage mode (Only STM32F429x/439x devices)
...
/**
  ...
  *                 is ready (STM32F429x/439x devices)
  *            @arg PWR_FLAG_ODSWRDY: This flag indicates that the Over-drive mode
  *                 switcching is ready (STM32F429x/439x devices) 
  *            @arg PWR_FLAG_UDRDY: This flag indicates that the Under-drive mode
  *                 is enabled in Stop mode (STM32F429x/439x devices)
...
/**
  ...
  *            @arg PWR_FLAG_UDRDY: Under-drive ready flag (STM32F429x/439x devices)
  * @retval None
  */
