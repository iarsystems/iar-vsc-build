...
export class ThriftProject implements ExtendedProject, Disposable {
    ...
    public getCStatOutputDirectory(config: string): Promise<string | undefined> {
        return this.performOperation(async() => {
            ...
            if (WorkbenchFeatures.supportsFeature(this.owner, WorkbenchFeatures.ExpandArgVars, fullConfig.targetId)) {
                return this.projectMgr.ExpandArgVars(outDir, this.context, config, false);
            } else {
                return outDir;
            }
        });
